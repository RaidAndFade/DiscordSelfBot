...
function DiscordClient(options) {
	...
	function getServerInfo(servArr, callback) {
		...
		for (var server=0; server<servArr.length; server++) {
			...
			self.servers[serverID].users = {};
			self.servers[serverID].rolesTMP = {};
			self.servers[serverID].roles = {};
			
			roleArr.forEach(function(role) {
				self.servers[serverID].rolesTMP[role.id] = role;
				self.servers[serverID].roles[role.id] = new Role(role);
			});
			//console.log(self.servers[serverID].roles); //debug//
			...
			memberArr.forEach(function(member) {
				...
				self.servers[serverID].users[member.user.id] = new User(self.servers[serverID].roles,member);
			});
	...
	function handleWSMessage(data, flags) {
		...
		switch (message.t) {
			case "READY":
				...
				break;
			case "MESSAGE_CREATE":
				message.d.guild_id = self.serverFromChannel(message.d.channel_id);
				//console.log(message);//debug//
				self.emit('message', message.d.author, message.d.channel_id, message.d.content, message);
				break;
			case "MESSAGE_UPDATE":
				message.d.guild_id = self.serverFromChannel(message.d.channel_id);
				self.emit('messageud', message.d.id, message.d.author, message.d.channel_id, message.d.content, message);
				break;
			case "MESSAGE_DELETE":
				message.d.guild_id = self.serverFromChannel(message.d.channel_id);
				self.emit('messagede', message.d.id, message.d.author, message.d.channel_id, message.d.content, message);
				break;
			case "PRESENCE_UPDATE":
				var server;
				...
				self.emit('presence', message.d.user.username, message.d.user.id, message.d.status, message.d.game ? message.d.game.name : null , message);
				break;
			case "USER_UPDATE":
				if (message.d) {
					for (var userItem in message.d) {
	...
	function User(rolelist, member){
		...
		this.rolesTMP = member.roles;
		this.roles = {};
		this.permissions = {};
		for (var i = 0; i < this.rolesTMP.length; i++) {
			if(this.rolesTMP[i]){
				roleid = this.rolesTMP[i];
				role = rolelist[roleid];
				this.roles[i] = role;
				this.permissions["GENERAL_CREATE_INSTANT_INVITE"] = 
					this.permissions["GENERAL_CREATE_INSTANT_INVITE"]==false
					&&role.permission_values.GENERAL_CREATE_INSTANT_INVITE!=false?role.permission_values.GENERAL_CREATE_INSTANT_INVITE:false;
				this.permissions["GENERAL_KICK_MEMBERS"] = 
					this.permissions["GENERAL_KICK_MEMBERS"]==false
					&&role.permission_values.GENERAL_KICK_MEMBERS!=false?role.permission_values.GENERAL_KICK_MEMBERS:false;
				this.permissions["GENERAL_BAN_MEMBERS"] = 
					this.permissions["GENERAL_BAN_MEMBERS"]==false
					&&role.permission_values.GENERAL_BAN_MEMBERS!=false?role.permission_values.GENERAL_BAN_MEMBERS:false;
				this.permissions["GENERAL_MANAGE_ROLE"] = 
					this.permissions["GENERAL_MANAGE_ROLES"]==false
					&&role.permission_values.GENERAL_MANAGE_ROLES!=false?role.permission_values.GENERAL_MANAGE_ROLES:false;
				this.permissions["GENERAL_MANAGE_CHANNELS"] = 
					this.permissions["GENERAL_MANAGE_CHANNELS"]==false
					&&role.permission_values.GENERAL_MANAGE_CHANNELS!=false?role.permission_values.GENERAL_MANAGE_CHANNELS:false;
				this.permissions["GENERAL_MANAGE_GUILD"] = 
					this.permissions["GENERAL_MANAGE_GUILD"]==false
					&&role.permission_values.GENERAL_MANAGE_GUILD!=false?role.permission_values.GENERAL_MANAGE_GUILD:false;
					
				//text
				this.permissions["TEXT_READ_MESSAGES"] = 
					this.permissions["TEXT_READ_MESSAGES"]==false
					&&role.permission_values.TEXT_READ_MESSAGES!=false?role.permission_values.TEXT_READ_MESSAGES:false;
				this.permissions["TEXT_SEND_MESSAGES"] = 
					this.permissions["TEXT_SEND_MESSAGES"]==false
					&&role.permission_values.TEXT_SEND_MESSAGES!=false?role.permission_values.TEXT_SEND_MESSAGES:false;
				this.permissions["TEXT_SEND_TTS_MESSAGE"] = 
					this.permissions["TEXT_SEND_TTS_MESSAGE"]==false
					&&role.permission_values.TEXT_SEND_TTS_MESSAGE!=false?role.permission_values.TEXT_SEND_TTS_MESSAGE:false;
				this.permissions["TEXT_MANAGE_MESSAGES"] = 
					this.permissions["TEXT_MANAGE_MESSAGES"]==false
					&&role.permission_values.TEXT_MANAGE_MESSAGES!=false?role.permission_values.TEXT_MANAGE_MESSAGES:false;
				this.permissions["TEXT_EMBED_LINKS"] = 
					this.permissions["TEXT_EMBED_LINKS"]==false
					&&role.permission_values.TEXT_EMBED_LINKS!=false?role.permission_values.TEXT_EMBED_LINKS:false;
				this.permissions["TEXT_ATTACH_FILES"] = 
					this.permissions["TEXT_ATTACH_FILES"]==false
					&&role.permission_values.TEXT_ATTACH_FILES!=false?role.permission_values.TEXT_ATTACH_FILES:false;
				this.permissions["TEXT_READ_MESSAGE_HISTORY"] = 
					this.permissions["TEXT_READ_MESSAGE_HISTORY"]==false
					&&role.permission_values.TEXT_READ_MESSAGE_HISTORY!=false?role.permission_values.TEXT_READ_MESSAGE_HISTORY:false;
				this.permissions["TEXT_MENTION_EVERYONE"] = 
					this.permissions["TEXT_MENTION_EVERYONE"]==false
					&&role.permission_values.TEXT_MENTION_EVERYONE!=false?role.permission_values.TEXT_MENTION_EVERYONE:false;
					
				//voice
				this.permissions["VOICE_CONNECT"] = 
					this.permissions["VOICE_CONNECT"]==false
					&&role.permission_values.VOICE_CONNECT!=false?role.permission_values.VOICE_CONNECT:false;
				this.permissions["VOICE_SPEAK"] = 
					this.permissions["VOICE_SPEAK"]==false
					&&role.permission_values.VOICE_SPEAK!=false?role.permission_values.VOICE_SPEAK:false;
				this.permissions["VOICE_MUTE_MEMBERS"] = 
					this.permissions["VOICE_MUTE_MEMBERS"]==false
					&&role.permission_values.VOICE_MUTE_MEMBERS!=false?role.permission_values.VOICE_MUTE_MEMBERS:false;
				this.permissions["VOICE_DEAFEN_MEMBERS"] = 
					this.permissions["VOICE_DEAFEN_MEMBERS"]==false
					&&role.permission_values.VOICE_DEAFEN_MEMBERS!=false?role.permission_values.VOICE_DEAFEN_MEMBERS:false;
				this.permissions["VOICE_MOVE_MEMBERS"] = 
					this.permissions["VOICE_MOVE_MEMBERS"]==false
					&&role.permission_values.VOICE_MOVE_MEMBERS!=false?role.permission_values.VOICE_MOVE_MEMBERS:false;
				this.permissions["VOICE_USE_VAD"] = 
					this.permissions["VOICE_USE_VAD"]==false
					&&role.permission_values.VOICE_USE_VAD!=false?role.permission_values.VOICE_USE_VAD:false;
			}
		}
