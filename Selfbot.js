...
utils={
	...
	getPerms: (event,user)=>{
		if(event.guild_id){
			...
			var userPermissions=0;
			var isGOwner = event.channel_id==event.user_id;
		}
		var perms = {};
...
bot.on('ready', function(rawEvent) {
	...
	utils.readyEvent = rawEvent;
});
bot.on('allUsers', function(){
	for(modf in Modules){
		Modules[modf].emit("ready",utils,utils.readyEvent);
	}
	delete utils.readyEvent;
...
bot.on('message', function(unusable, unusable2, channelId, message, rawEvent) {
	...
	if(rawEvent.d.author.id!=owner)return;
	init=utils.chanData[channelId].settings.comInit;
	data.commandReplies[rawEvent.d.id]=rawEvent.d.id;
...
bot.on('messageUpdate', function(nothing,rawEvent){
	...
	if(rawEvent.author.id!=owner)return;
	data.commandReplies[rawEvent.id]=rawEvent.id;
	//console.log(message);
	init=utils.chanData[channelId].settings.comInit;
	if((init!=""&&message.startsWith(init))||message.startsWith("<@"+utils.ownId+">")||message.startsWith("<@!"+utils.ownId+">")){
